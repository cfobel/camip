...
import numpy as np
from camip.place import (place_from_hdf, placer_to_block_positions_df,
                         save_placement)
...
def parse_args(argv=None):
    ...
    parser.add_argument('-s', '--seed', default=np.random.randint(100000),
                        type=int)
    parser.add_argument(dest='hdf_net_file', type=path)
...
if __name__ == '__main__':
    ...
    schedule, placer, params, place_stats = place_from_hdf(
        args.hdf_net_file, args.seed,
    ...
    if args.output_path is None and args.timing:
        args.output_path = ('placed-%(net_file_namebase)s-s%(seed)s-%(block_positions_sha1)s' +
                            '-e%.2f-w%.2f-I%d.h5' % (args.criticality_exp,
                                                     ...
                                                     args.io_capacity))
    hdf_output_path = save_placement(args.hdf_net_file.namebase,
